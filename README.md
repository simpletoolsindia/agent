...
The one-shot CLI now follows the Oh My Pi cockpit style: rounded panels, segmented metric chips, stage timelines, shimmer text, and animated `AI running` status. The TUI opens a modern OMP-inspired setup cockpit when key connection settings are missing. Use it to edit model name, OpenAI-compatible server URL, API key, approval mode, `agent.md`, and `skills.md` before the chat starts. The setup screen groups profile cards, workspace context, command shortcuts, live validation, active profile chips, docs count, a live spinner, and bounded status/progress rows for narrow terminals. During model/tool work the chat UI shows rounded message cards, `You` / `Assistant · reply` / `Think · live` labels, boxed live reasoning with `Reasoning` separators, inline tool progress, parallel tool counts, scroll hints, stage timelines, shimmering active labels, and cool cyan/blue/green/amber progress gradients adapted from `vraj00222/tui`'s `ProgressBar` pattern. Tool cards use the requested output box: command/status row, `├─── Output ─` / `Diff` / `Live status` separators, bounded rows, and footers like `⟦Wall: 3.11s | Timeout: 120s⟧`; todo cards render phase trees, and subagent cards update live with role, goal, and running status before the final summary. `Esc` or `Ctrl+C` during model/tool work now interrupts the active stream immediately, aborts any running subagent, and opens the prompt so the next user message can redirect the agent or queue the next task. Rotating suggestions call out when to use `search`, `read`, `update`, `write`, `bash`, `/settings`, `/agents`, `/sessions`, and `/compact`. The title uses a compact status-line style with workspace, model, approval mode, context usage, and slash-command hints. `--context-size` defaults to `32768`.
